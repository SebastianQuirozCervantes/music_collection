...
class UsersRepository extends CommonRepository {
  ...
  async findAllBackOfficeUsers(query) {
    ...
    if (query && Object.keys(query).length !== 0) {
      builder = await this.filterUsersByQuery(builder,
        query);
    } else {
      builder = builder.orderBy(created_at_,
        'DESC');
    }
  ...
  async findUserPropertiesById(id_user) {
    const builder = getConnection()
      ...
      .from('user',
        'user')
      .leftJoin('user.person',
        'person')
      .leftJoin('user.properties',
        'properties')
      .leftJoin(
